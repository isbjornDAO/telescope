...
export const GET = handle(async () => {
  ...
  const [season, nodes, anchors, elders, crews, factions, regions, activeIntents, recentMatches, vouches, topFactions, victor] = await Promise.all([
    ...
    prisma.region.findMany({ select: { name: true, slug: true, lat: true, lng: true, standing: true, _count: { select: { users: true, crews: true } } } }),
  ...
  return ok(
    {
      ...
      regions: regions.map((r) => ({ name: r.name, slug: r.slug, lat: r.lat, lng: r.lng, standing: r.standing, nodes: r._count.users, crews: r._count.crews, anchors: anchorMap.get(r.slug) ?? 0 })),
